...
if (isset($_POST['type']) && isset($_POST['user'])){
	...
	if($_POST['type'] == "friend"){
	    ...
	    if($friend_count[0] > 5000){
            mysqli_close($db_conx);
	        echo "$user currently has the maximum number of friends and cannot accept more.";
	        exit();
        } else if($blockcount1[0] > 0){
            mysqli_close($db_conx);
	        echo "$user has you blocked, so you cannot become friends with them.";
	        exit();
        } else if($blockcount2[0] > 0){
            mysqli_close($db_conx);
	        echo "You must first unblock $user in order to become friends with them.";
	        exit();
		    ...
		    mysqli_close($db_conx);
	        echo "You have already sent a request to $user.";
	        exit();
	    } else if ($row_count4[0] > 0) {
		    mysqli_close($db_conx);
	        echo "$user has requested to become friends with you first. Please check your notifications.";
	        exit();
	    } else {
	    ...
	    if ($row_count1[0] > 0) {
			...
			mysqli_close($db_conx);
	        echo "You are not friends with $user, therefore we cannot unfriend you.";
	        exit();
		}
	}
}
